export async function summarizeCurrentPage() {
  ...
  try {
    ...
    reflectionContent.innerText = '🔄 Summarizing current page...';
    ...
    if (!result?.text) { reflectionContent.innerText = 'No readable content found on this page.'; return; }
    if (!chrome.ai?.summarizer) {
      const sentences = result.text.split(/(?<=[.!?])\s+/).slice(0, 3).join(' ');
      reflectionContent.innerText = `📰 ${result.title}\n\n${sentences}`;
      return;
    }
    const summary = await chrome.ai.summarizer.summarize({ input: `Title: ${result.title}\n\nContent:\n${result.text}\n\nSummarize in 3 concise bullet points.` });
    reflectionContent.innerText = `📰 ${result.title}\n\n${summary.output}`;
  } catch (e) {
    reflectionContent.innerText = `❌ Summarization failed: ${e.message}`;
  } finally {
